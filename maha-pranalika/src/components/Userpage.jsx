...
const handleResolvedMSME=(id)=>{
  ...
  <div className="form-section visa-assistance-section">
    ...
    {user.visa_assistance.map((entry) => (
      <div key={entry._id} className="form-card">
        {/* Personal Info */}
        <p><strong>Full Name:</strong> {entry.personal_information?.fullName}</p>
        <p><strong>Date of Birth:</strong> {entry.personal_information?.dob}</p>
        <p><strong>Gender:</strong> {entry.personal_information?.gender}</p>
        <p><strong>Passport Number:</strong> {entry.personal_information?.passportNumber}</p>
        <p><strong>Passport Expiry:</strong> {entry.personal_information?.passportExpiry}</p>
        <p><strong>Phone Number:</strong> {entry.personal_information?.phoneNumber}</p>
        <p><strong>Email:</strong> {entry.personal_information?.email}</p>
        <p><strong>Address:</strong> {entry.personal_information?.address}</p>

        {/* Consultation Details */}
        <p><strong>Visa Types:</strong> {entry.consultation_details?.visaTypes?.join(", ")}</p>
        {entry.consultation_details?.otherVisaType && (
          <p><strong>Other Visa Type:</strong> {entry.consultation_details?.otherVisaType}</p>
        )}
        <p><strong>Preferred Countries:</strong> {entry.consultation_details?.preferredCountries?.join(", ")}</p>
        {entry.consultation_details?.otherPreferredCountry && (
          <p><strong>Other Preferred Country:</strong> {entry.consultation_details?.otherPreferredCountry}</p>
        )}
        <p><strong>Referrer Name:</strong> {entry.consultation_details?.referrerName}</p>

        {/* Education & Experience */}
        {entry.education_experience?.qualification?.map((q, idx) => (
          <p key={idx}>
            <strong>Qualification:</strong> {q.qualification} from {q.institute} ({q.year})
          </p>
        ))}
        <p><strong>Current Occupation:</strong> {entry.education_experience?.currentOccupation}</p>
        <p><strong>Years of Experience:</strong> {entry.education_experience?.yearsOfExperience}</p>
        ...
        {entry.languageProficiency?.languages?.map((lang, idx) => (
          <div key={idx}>
            <p>
              ...
              Speaking: {lang.speaking ? "Yes" : "No"} |
              Reading: {lang.reading ? "Yes" : "No"} |
              Writing: {lang.writing ? "Yes" : "No"}
            </p>
            <p><strong>Exams Taken:</strong> {lang.examsTaken}</p>
          </div>
        ))}

        {/* Additional Details */}
        <p><strong>Applied Before:</strong> {entry.addtionalDetails?.previouslyappliedforvisa}</p>
        <p><strong>Visa Rejections:</strong> {entry.addtionalDetails?.anyvisarejections}</p>
        <p><strong>Legal Issues:</strong> {entry.addtionalDetails?.anylegailissues}</p>
        ...
        {/* Declaration */}
        <p><strong>Declaration Signed:</strong> {entry.declaration?.declared ? "Yes" : "No"}</p>
        <p><strong>Signature:</strong> {entry.declaration?.signature}</p>
        <p>
          <strong>Submitted On:</strong>{" "}
 ...
 {user.msme?.length > 0 && (
  <div className="form-section msmeclusters-section">
    ...
    {user.msme.map((entry) => (
      <div key={entry._id} className="form-card">
        {/* Basic Info */}
        <p><strong>Client Name:</strong> {entry.basic_info?.client_name}</p>
        <p><strong>Primary Product:</strong> {entry.basic_info?.primary_product}</p>
        <p><strong>Total Leads:</strong> {entry.basic_info?.total_no_of_leads}</p>
        <p><strong>Lead Name:</strong> {entry.basic_info?.name_of_lead}</p>
        <p><strong>Organisation Types:</strong> {entry.basic_info?.type_of_organisation?.join(", ")}</p>
        <p><strong>Lead Entity:</strong> {entry.basic_info?.lead_entity}</p>
        <p><strong>Date of Establishment:</strong> {entry.basic_info?.doe}</p>
        <p><strong>Address:</strong> {entry.basic_info?.address}</p>
        <p><strong>City:</strong> {entry.basic_info?.city}</p>
        <p><strong>District:</strong> {entry.basic_info?.district}</p>
        <p><strong>State:</strong> {entry.basic_info?.state}</p>
        <p><strong>Pincode:</strong> {entry.basic_info?.pincode}</p>

        {/* Contact Info */}
        <p><strong>Contact Person:</strong> {entry.contact_info?.contact_person}</p>
        <p><strong>Designation:</strong> {entry.contact_info?.designation}</p>
        <p><strong>Phone:</strong> {entry.contact_info?.phone}</p>
        <p><strong>Email:</strong> {entry.contact_info?.email}</p>

        {/* Cluster Details */}
        <p><strong>Participants:</strong> {entry.cluster_details?.no_of_participating}</p>
        <p><strong>Avg. Years in Business:</strong> {entry.cluster_details?.average_years}</p>
        <p><strong>Total Employment:</strong> {entry.cluster_details?.total_employment}</p>
        <p><strong>Common Challenges:</strong> {entry.cluster_details?.common_challenges?.join(", ")}</p>
        <p><strong>Key Interventions:</strong> {entry.cluster_details?.key_interventions}</p>
        ...
        {/* Declaration */}
        <p><strong>Declared:</strong> {entry.declaration?.declared ? "Yes" : "No"}</p>
        <p><strong>Signature:</strong> {entry.declaration?.signature}</p>
        <p><strong>Declaration Date:</strong> {entry.declaration?.declaration_date
          ? new Date(entry.declaration.declaration_date).toLocaleDateString()
          : "N/A"}</p>
